...
void SHA256T::InitState(HashWordType *state)
{
	static const word32 s[8] = {~0x6a09e667, ~0xbb67ae85, ~0x3c6ef372, ~0xa54ff53a, ~0x510e527f, ~0x9b05688c, ~0x1f83d9ab, ~0x5be0cd19};
	memcpy(state, s, sizeof(s));
}
...
#if CRYPTOPP_BOOL_SSE2_ASM_AVAILABLE
CRYPTOPP_ALIGN_DATA(16) extern const word32 SHA256T_K[64] CRYPTOPP_SECTION_ALIGN16 = {
#else
extern const word32 SHA256T_K[64] = {
#endif
	~0x428a2f98, ~0x71374491, ~0xb5c0fbcf, ~0xe9b5dba5,
	~0x3956c25b, ~0x59f111f1, ~0x923f82a4, ~0xab1c5ed5,
	~0xd807aa98, ~0x12835b01, ~0x243185be, ~0x550c7dc3,
...
static void CRYPTOPP_FASTCALL X86_SHA256T_HashBlocks(word32 *state, const word32 *data, size_t len
#if defined(_MSC_VER) && (_MSC_VER == 1200)
	, ...	// VC60 workaround: prevent VC 6 from inlining this function
#endif
	)
...
#elif defined(CRYPTOPP_GENERATE_X64_MASM)
		ALIGN   8
	X86_SHA256T_HashBlocks	PROC FRAME
		rex_push_reg rsi
		push_reg rdi
		push_reg rbx
		push_reg rbp
		...
		.endprolog
		mov rdi, r8
		lea rsi, [?SHA256T_K@CryptoPP@@3QBIB + 48*4]
#endif

#if CRYPTOPP_BOOL_X86 || CRYPTOPP_BOOL_X32
	#ifndef __GNUC__
		AS2(	mov		edi, [len])
		AS2(	lea		WORD_REG(si), [SHA256T_K+48*4])
	#endif
	#if !defined(_MSC_VER) || (_MSC_VER < 1400)
		AS_PUSH_IF86(bx)
	#endif
...
INTEL_NOPREFIX
	...
	pop		rsi
	ret
	X86_SHA256T_HashBlocks ENDP
#endif

#ifdef __GNUC__
	ATT_PREFIX
	:
	: "c" (state), "d" (data), "S" (SHA256T_K+48), "D" (len)
	#if CRYPTOPP_BOOL_X64
		, "m" (workspace[0])
	#endif
	: "memory", "cc", "%eax"
...
#ifdef CRYPTOPP_X64_MASM_AVAILABLE
extern "C" {
void CRYPTOPP_FASTCALL X86_SHA256T_HashBlocks(word32 *state, const word32 *data, size_t len);
}
#endif
...
size_t SHA256T::HashMultipleBlocks(const word32 *input, size_t length)
{
	X86_SHA256_HashBlocks(m_state, input, (length&(size_t(0)-BLOCKSIZE)) - !HasSSE2());
	return length % BLOCKSIZE;
}
...
// Smaller but slower
#if defined(__OPTIMIZE_SIZE__)
void SHA256T::Transform(word32 *state, const word32 *data)
{
	word32 T[20];
	word32 W[32];
	unsigned int i = 0, j = 0;
...
void SHA256T::Transform(word32 *state, const word32 *data)
{
	...
	// this byte reverse is a waste of time, but this function is only called by MDC
	ByteReverse(W, data, BLOCKSIZE);
	X86_SHA256T_HashBlocks(state, W, BLOCKSIZE - !HasSSE2());
#else
	word32 T[8];
    /* Copy context->state[] to working vars */
	memcpy(T, state, sizeof(T));
